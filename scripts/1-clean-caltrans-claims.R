...
caltrans_data_for_table <- caltrans_data_clean %>% 
  filter(INCIDENT_YEAR == '2018' |INCIDENT_YEAR == '2019' | INCIDENT_YEAR == '2020' | INCIDENT_YEAR == '2021' | INCIDENT_YEAR == '2022' | INCIDENT_YEAR == '2023') %>% 
  select(DATE_NEW, `DATE PRESENTED`, COUNTY, DISTRICT, `ROUTE CLEAN`, `STATUS CLEAN`) %>% 
  mutate(COUNTY = case_when(COUNTY == "?" ~ NA,
                            COUNTY == "405" ~ NA,
                            COUNTY == "57" ~ NA,
                            COUNTY == "73" ~ NA,
                            TRUE ~ COUNTY)) %>% 
  rename(`INCIDENT DATE` = DATE_NEW,
         `CLAIM DATE` = `DATE PRESENTED`,
         `ROUTE` = `ROUTE CLEAN`,
         `STATUS` = `STATUS CLEAN`) %>% 
  arrange(COUNTY)

write.csv(caltrans_data_for_table, "caltrans_data_for_table.csv", row.names = FALSE)
